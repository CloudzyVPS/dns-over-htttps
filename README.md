...
- **Endpoint:** `/dns-query?dns=...`
- **Method:** `GET`
- **Query parameter:** `dns` (base64url-encoded DNS message)
...
### 3. JSON API GET (Cloudflare/Google style)

- **Endpoint:**
  - Cloudflare: `/dns-json?name=example.com&type=A`
  - Google: `/resolve?name=example.com&type=A`
  - Quad9: `/dns-query?name=example.com&type=A&ct=application/dns-json`
- **Method:** `GET`
- **Query parameters:** `name` (domain), `type` (record type), etc.
