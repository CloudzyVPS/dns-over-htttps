...
describe('validation', () => {
  describe('validateBase64Url', () => {
    ...
    it('rejects invalid base64url strings', () => {
      expect(validateBase64Url('test+++')).toBe(false); // contains +
      expect(validateBase64Url('test///')).toBe(false); // contains /
      expect(validateBase64Url('test===')).toBe(false); // contains =
      expect(validateBase64Url('test space')).toBe(false); // contains space
      expect(validateBase64Url('')).toBe(false); // empty
    });
  });
  ...
  describe('validateDnsType', () => {
    it('accepts valid numeric DNS types', () => {
      expect(validateDnsType('1')).toBe(true); // A
      expect(validateDnsType('28')).toBe(true); // AAAA
      expect(validateDnsType('65535')).toBe(true); // max
    });
    ...
    it('accepts valid DNS type names', () => {
      ...
      expect(validateDnsType('a')).toBe(true); // case insensitive
    });
